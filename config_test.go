...
func TestPublicationConfigValidateRejectsInvalidValues(t *testing.T) {
	tests := []struct {
		...
		{name: "udp congestion on quic", cfg: PublicationConfig{RemoteAddr: "127.0.0.1:1", UDPCongestionControl: NewAIMDUDPCongestionControl()}},
...
func TestSubscriptionConfigValidateRejectsInvalidValues(t *testing.T) {
	tests := []struct {
		...
		{name: "negative udp nak retry interval", cfg: SubscriptionConfig{LocalAddr: "127.0.0.1:0", UDPNakRetryInterval: -time.Nanosecond}},
		...
		{name: "driver data ring too small for shared image message", cfg: SubscriptionConfig{LocalAddr: "127.0.0.1:0", DriverDataRingCapacity: minDriverSubscriptionImageRecordBytes - 1}},
