...
| Term buffers and zero copy | Aeron publications claim into shared log buffers and subscribers scan fragments/images from those buffers. | Bunshin has term positions, claim APIs, mmap-backed publication term buffers for driver-managed publications, and per-subscription mmap shared images for external polling. | Make mmap term buffers the actual cross-process publication data path and deepen subscription images into a fuller log-buffer/image lifecycle with fragment, block, raw polling, unblock, and position-counter semantics. |
