...
External publication payloads still cross the external-driver boundary inside IPC commands. Promoting external publication sends to client-writable mmap log buffers, with driver-owned producer positions and back-pressure metadata, is the next Aeron-semantic data-path gap.
