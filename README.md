...
- Reliable send over `quic-go` streams with application-level ACK frames; UDP sends Bunshin frames to unicast, multi-destination, or multicast destinations with HELLO setup, receiver STATUS, NAK repair/retry, RTT feedback, sender destination and receiver peer diagnostics, and application-level ACK/ERROR responses.
...
- Optional AIMD congestion-window policy for the explicit UDP transport.
...
- Embeddable media driver with lifecycle counters, mmap-backed publication term buffers, and mmap shared-image polling for external subscriptions.
